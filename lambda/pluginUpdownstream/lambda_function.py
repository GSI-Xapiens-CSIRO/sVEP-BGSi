...
TRANSCRIPT_ID_PATTERN = re.compile('transcript_id\\s\\"(\\w+)\\";', re.IGNORECASE)
...
def query_updownstream(chrom, pos, alt, transcripts, ref_chrom):
    up = int(pos) - 5000
    down = int(pos) + 5000
    results = []
    loc = f"{ref_chrom}:{up}-{down}"
    ...
    for data in main_data:
        if TRANSCRIPT_ID_PATTERN.search(data).group(1) in transcripts:
            continue
        metadata = data.split("\t")
        info = dict(strict_split(item) for item in metadata[8].split("; "))
        ...
        if support_level_set:
            fields.append(
                info.get("transcript_support_level", ".").rstrip(";").strip('"')
            )
        ...
        for row in sns_data:
            ...
            if transcripts:
                results = query_updownstream(
                    chrom, pos, alt, list(set(transcripts)), ref_chrom
                )
            if results:
